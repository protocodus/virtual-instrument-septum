// Renders the demonstration WAVs under Docs/audio from the same
// JUCE-free engine the plug-in runs, so a demonstration cannot drift away
// from what Septum actually sounds like. No samples or external
...
int runSmokeTest (const std::filesystem::path& directory)
{
    ...
    std::printf ("Septum demo renderer smoke test passed (peak %.3f).\n",
                 take.peak());
    return 0;
}
...
// from the generated WAVs.
constexpr const char* peaksTableBegin =
    "<!-- peaks-table-begin: regenerated by SeptumRenderDemos;"
